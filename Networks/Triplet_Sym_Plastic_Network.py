...
def Network_get_Stats(W = 0.0,
   ...
   # NSigs = 8  # Number of input signals.
    ...
    dApre1/dt=-Apre1/tau_stdp : 1 (event-driven)
    dApre2/dt=-Apre2/(5*tau_stdp) : 1 (event-driven)
    dApost1/dt=-Apost1/tau_stdp : 1 (event-driven)
    dApost2/dt=-Apost2/(5*tau_stdp) : 1 (event-driven)
    '''
    ...
    Pre_eq = '''
             ...
             Apre1 += 1.
             Apre2 += 1.
             w_minus = eta_e*A_LTD*Apre2*Apost1
             w = clip(w - w_minus, 0.001, gmax)
             w = (1-eta_norm)*w +  eta_norm*w*NE*w_target/sumwe_post
             '''
    # equations executed only when a postsynaptic spike occurs
    Post_eq = '''
              Apost1 += 1.                                                                                        
              Apost2 += 1.                                                                                        
              w_plus = eta_e*A_LTP*Apre1*Apost2
    ...
    con_ie = Synapses(Pi, N, model=eqs_stdp_inhib,
                      on_pre='''Apre += 1.
                             ...
                             w = (1-eta_norm)*w +  eta_norm*w*c_i*NI*w_target/sumwi_post
                             g_gaba += gBarIn*w*nS''',
                      on_post='''Apost += 1.
